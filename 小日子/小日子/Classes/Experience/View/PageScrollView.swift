...
class PageScrollView: UIView {
    ...
    var headData: SMExperienDataModel? {
        didSet {
            ...
            if headData?.head!.count >= 0 {
                pageControl.numberOfPages = (headData?.head!.count)!
    ...
    private func updatePageScrollView() {
        for var i = 0; i < imageScrollView.subviews.count; i++ {
            ...
            imageView.tag = index
            if headData?.head!.count > 0 {
                imageView.sd_setImageWithURL(NSURL(string: (headData?.head![index].adurl!)!), placeholderImage: placeholderImage)
            }
        }
